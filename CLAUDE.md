...
12. **Comment the why, not the what.** Every non-obvious design decision must have a concise inline comment explaining *why* the code is written that way — not what it does (the code shows that). This includes: security trade-offs (fail-open vs fail-closed), rate limiting thresholds and their rationale, anti-enumeration patterns, compliance requirements (TCPA, consent), async/sync bridging patterns, pagination strategies, data model choices (e.g., timestamp vs separate table), and any deliberate deviation from the "obvious" approach. Comments are written during implementation, not added later. A new developer reading the code should never need to ask "but why?" about a design choice.
13. **Module docstrings on every new file.** Every new Python file must include a module-level docstring (first line) that describes the file's purpose in one sentence. This is done at file creation time, not deferred.
